...
> **각 단계마다 근거를 같이 적었다.**
>
> **모르는 용어가 나오면** → [레퍼런스_IsaacSim_용어사전.md](레퍼런스_IsaacSim_용어사전.md) ·
> **도구가 안 잡히면** → [레퍼런스_IsaacSim_도구.md](레퍼런스_IsaacSim_도구.md)
>
> **다음 단계** — 로봇에 그리퍼를 붙이고 클로즈드루프를 푸는 건
> [GUI_실습_매니퓰레이터_그리퍼_어셈블_클로즈드루프_최적화.md](GUI_실습_매니퓰레이터_그리퍼_어셈블_클로즈드루프_최적화.md)
...
📖 **전체 레퍼런스 → [도구 카드 §1 Gain Tuner](레퍼런스_IsaacSim_도구.md#1-gain-tuner)** — 물리 모델, 드라이브 모드 4종, 테스트 파형 파라미터, 저장 경로, 로봇이 드롭다운에 안 뜰 때
